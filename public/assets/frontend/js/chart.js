$(document).ready(function () {
	$(".1-year").click(function () {
		...
		$('li.box-link').removeClass('active')
		$(".1-year").parent('li').addClass('active')
...
$(document).ready(function () {
	$(".1-month").click(function () {
		...
		var type = $(this).attr('data-value');
		$('li.box-link').removeClass('active')
		$(".1-month").parent('li').addClass('active')
		$("#chart_div").remove();
		$('.chart-section').prepend(`<div id="chart_div"></div>`);
		$.ajax({
			type: 'GET',
...
$(document).ready(function () {
	$(".3-month").click(function () {
		...
		var type = $(this).attr('data-value');
		$('li.box-link').removeClass('active')
		$(".3-month").parent('li').addClass('active')
		$("#chart_div").remove();
		$('.chart-section').prepend(`<div id="chart_div"></div>`);
		$.ajax({
			type: 'GET',
...
function createChart(data, type) {
	...
	if (type == "year") {
		google.charts.setOnLoadCallback(drawChartYear($.parseJSON(data)));
	}
	else if (type == "all") {
		google.charts.setOnLoadCallback(drawChartAll($.parseJSON(data)));
	}
	else if (type == "month") {
		google.charts.setOnLoadCallback(drawChartMonth($.parseJSON(data)));
	}
}

function drawChartYear(value) {
...
function drawChartMonth(value) {

	// console.log(value)
	var areaStyle = 'fill-color: #ffeb3b; stroke-color: #b71c1c; stroke-width: 8;';
	var data = google.visualization.arrayToDataTable(value);
	var options = {
		eries: { //Create 2 separate series to fake what you want. One for the line             and one for the points
			0: {
				color: '#A6FAFA',
				lineWidth: 2
			},
